		...
		try:
			if 1 in missing:
				...
				if os_name == 'Windows':
					...
					out = Netsys.asset_downloader(link, AboutApp.temp_dir, 'who_r_u.mp3', '0306x1', '0306x2', False, err_print=err_print)
				...
				if out: del missing[missing.index(1)]

			if 2 in missing:
				message = "was DLing 'wl-page.html'"
				link = AboutApp.cloud_html_temp_link
				out = Netsys.asset_downloader(link, AboutApp.temp_dir, 'wl-page.html', '0306x7', '0306x8', True, err_print=err_print, proxy=[AboutApp.cloud_html_temp_link_proxy,])
				err_print = out and err_print
				if out: del missing[missing.index(2)]

			"""
			if 3 in missing:
				message = "was DLing 'updateL.ext'"
				link = AboutApp.cloud_data_link
				out = Netsys.asset_downloader(link, AboutApp.temp_dir, 'updateL.ext', '0306x3', '0306x4', True, err_print=err_print)
				if out:
			...
			if 4 in missing:
				...
				out = Netsys.asset_downloader(link, AboutApp.temp_dir, 'updateG.ext', '0306x5', '0306x6', True, err_print=err_print)
				if out:
			...
			if 5 in missing:
				...
				for name, link in links.items():
					# link = links[name]
					out = Netsys.asset_downloader(link, AboutApp.temp_dir+'scripts/', name, '0306x9', '0306xA', True, err_print=err_print)
					err_print = out and err_print
			...
			if 6 in missing:
				...
				for name, link in links.items():
					# link = links[name]
					out = Netsys.asset_downloader(link, AboutApp.temp_dir+'assets/', name, '0306xB', '0306xC', False, err_print=err_print)
					err_print = out and err_print"""
			...
			if 7 in missing:
				...
				out = Netsys.asset_downloader(link, AboutApp.temp_dir, name, '0306xB', '0306xC', True, err_print=err_print)
				if out:
					import zipfile
					...
					del missing[missing.index(7)]

			if missing:
				xprint("/rh/Error code: 0306x", missing,"\nNo internet connection!/=/\nPlease Connect to internet on 1st run")
...
class Netsys_ :  # fc=0800
	...
	def get_page(self, link, referer=False, header=None, cache=False, failed=False, do_not_cache=True,
		...
		return page
	def asset_downloader(self, link:str, file_loc:str, filename:str, server_error_code:str, internet_error_code:str, overwrite:bool, err_print=True, allow_old=True, proxy=[]):  # fc=080E
		"""
...
class ProjectType_ :  # fc=0P00
	...
	def __default__(self):  # fc=0P02
		...
		# download threads
		self.dl_thread_count = 10  # number of download threads
		self.dl_threads = []  # list of download threads
		...
		self.first_page = None  # CBZ or webpage location
	...
	def store_current_data(self): #fc=????
		...
		dataset= {"Project": self.Project,
			...
			'dl_thread_count': self.dl_thread_count,
	...
	def store_current_data_old(self): # fc=????
		...
		Fsys.writer(self.Project + '.wlproj', 'a', 'file_formats = %s\n' % str([self.file_types]),
		            AboutApp.leach_projects, '0M05')
		Fsys.writer(self.Project + '.wlproj', 'a', 'dl_thread_count = %s\n' % str(self.dl_thread_count),
		            AboutApp.leach_projects, '0M05')
	...
	def check_old_proj_file(self):  # fc=0P05
		...
		try:
			...
			if 'dl_threads' in loaded_data_set:
				self.dl_thread_count = loaded_data_set['dl_threads']
	...
	def start_link_index(self, threads=None):
		"""Non breaking link index
		returns after firing the threads"""
		self.index_threads = threads or self.index_threads
		
		len_sub_links = len(self.sub_links)

		self.all_list = All_list_type(len_sub_links)
		...
		for i in range(self.index_threads):
			index_thread_list.append(
				Process(target=self.generic_list_writer, args=(self.index_threads, i)))
			index_thread_list[i].start()


	def run_link_index(self, threads=None):
		self.index_threads = threads or self.index_threads
		...
		try:
			...
			leach_logger(log(["0P0Fx0", self.Project, e.__class__.__name__, e]), UserData.user_name)
			self.break_all = True
			time.sleep(5)
			exit(0)
	...
	def downloader(self, part, is_error=False, partitions=None):  # fc=0P0B
		if partitions is None:
			partitions = self.dl_thread_count
	...
	def retry_errors(self):  # fc=0P0C
		...
		nnn = [i for i in range(self.dl_thread_count)]
		if not all(i in self.error_triggers for i in nnn):
			return 0
		...
		##########################################
		# MAKE SURE MANGAFREAK PAGE IS MADE BEFORE DONE
		if 'mangafreak' in self.sp_flags:
			self.first_page = self.make_html()
...
\u29bf otherwise just leave an Enter
  /gh/>>/=/ """, end='')
	...
	def run_download(self, thread_count=None):
		"""unlike Main(), this is non breaking, will run by thread
		and return on fire
		
		So need to kill other way round"""

		
		if not 'mangafreak' in self.sp_flags:
			self.first_page = self.P.make_html()

		self.dl_thread_count = thread_count or self.dl_thread_count
		self.dl_threads = []

		for i in range(self.P.dl_thread_count):
			self.dl_threads.append(Process(target=self.downloader, args=[i]))
			self.dl_threads[i].start()

		
		if config.sp_arg_flag['max dlim'] != 0:
			self.tictoc = time.time()
			config.sp_arg_flag['chunk_size'] = int(config.sp_arg_flag['max dlim'] * (1024/self.dl_thread_count)) + 1
...
/g/  >> /=/""" % (' /h/[Recommended]/=/' if self.P.has_missing == True else ''), extra_opt=('b', 'fresh', 'f', 'cbz'),
				...
				if temp == 'browser' or temp == 'cbz':
					...
					if temp == 'browser':
						try:
							self.P.first_page = self.P.make_html()
							if not self.P.first_page:
						...
						try:
							self.P.first_page = self.P.make_cbz()
							print('CBZ Created in "%s"' % self.P.first_page)
						except (KeyboardInterrupt, EOFError):
