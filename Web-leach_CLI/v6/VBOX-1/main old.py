...
class AppConfig_(DefaultConfig):     #fc=0300
	...
	def god_mode(self, missing = None):     #fc=0306
		...
		try:
			...
			xprint("/rh/Error code: %s\nNo internet connection!/=/\nRunning offline mode"%ecode)
			leach_logger(log([ecode, Netsys.hdr(current_header, '0306'), link, e.__class__.__name__, e]), 'lock')
			time.sleep(3)
			return 'offline'
...
LinkErrors = (requests.exceptions.MissingSchema, requests.exceptions.InvalidSchema,
...
class Netsys_:     #fc=0800
	...
	def check_link_active(self, link, f_code='????', timeout=None, no_log = False):     #fc=0806
	...
	def remove_noscript(self, content):     #fc=080B
		...
		if isinstance(content, bytes):
			if b'<noscript>' in content: return re_sub(b"(?i)(?:<noscript>)(?:.|\n)*?(?:<\/noscript>)", b'', content)
		elif isinstance(content, str):
			if '<noscript>' in content: return re_sub("(?i)(?:<noscript>)(?:.|\n)*?(?:<\/noscript>)", '', content)
...
# working
class Sub_links_type:     #fc=xxxx
	""" Data structure for sub_links and sub_dirs"""
	def __init__(self, sub_dirs=[], sub_links=[], sub_names=[]):
		self.sub_dirs = sub_dirs
		self.sub_links = sub_links
		self.sub_names = sub_names

	def add_link(self, link):     #fc=xxxx
		"""adds a link to the sub_links list"""
...
class CachedData_:     #fc=0C00
	...
	def add_webpage(self, url, response):
		...
		__x = Cached_Response(status_code=response.status_code, headers=response.headers, content=response.content, encoding=response.encoding, url = response.url)
...
\u29bf otherwise just leave an Enter
  /gh/>>/=/ """, end='')
	...
	def dl_page(self, link = None, referer = False, header=None, cache = False, failed = False, do_not_cache = True, session = None):     #fc=0P0H
	...
	def clean_unknown_files(self):     #fc=0P0I
		...
		for dirpath, dirnames, filenames in os.walk(self.download_dir):
			...
			for filename in filenames:
				if filename=='index.html' and dp == self.Project:
					continue
	...
	def nhentai_link(self):     #fc=0P0N
		...
		if page:
			...
			title = Datasys.trans_str(parse.unquote(html_unescape(title)), {'/\\|:*><?': '-', '"':"'"}).strip()[:config.dir_limit]

			self.update_sub_dirs(Datasys.trans_str(parse.unquote(html_unescape(title)), {'/\\|:*><?': '-', '"':"'"}).strip()[:config.dir_limit], 0)
...
class Main:     #fc=0M00
	...
	def make_required_dirs(self):     #fc=0M03
		...
		for i in [AboutApp.download_dir, AboutApp.data_dir, AboutApp.temp_dir, AboutApp.leach_projects, AboutApp.cached_webpages_dir]:
			if not os_exists(i):
				makedirs(i)
...
/gh/  >> /=/"""%self.P.Project, extra_opt = ('f', 'fresh'), extra_return = ('fresh', 'fresh'))
			...
			if self.link_indexed == False:
				...
				try:
					...
					for i in range(self.P.index_threads):
						index_thread_list.append(Process(target=self.P.generic_list_writer, args=(self.P.index_threads, i)))
						index_thread_list[i].start()

					while any([i.is_alive() for i in index_thread_list]):
						if self.P.break_all == True:
							return False
						time.sleep(0.3)
		...
		Fsys.writer(self.P.Project + '.wlproj', 'a', f'magic_number = "{UserData.user_ip["ip"]}"\n',           AboutApp.leach_projects, '0M05')
		Fsys.writer(self.P.Project + '.wlproj', 'a', f'server_version = "{config.server_version}"\n',    AboutApp.leach_projects, '0M05')
		Fsys.writer(self.P.Project + '.wlproj', 'a', f'get_html_title = {self.P.get_html_title}\n',    AboutApp.leach_projects, '0M05')
...
if __name__ == '__main__':
	...
	while Keep_main_running:
		CachedData.clean_cached_webpages()
		main.make_required_dirs()
		try:
