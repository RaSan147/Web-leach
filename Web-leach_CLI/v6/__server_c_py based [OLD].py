			...
			'content="text/html; charset=%s">' % enc)
				...
				for i in range(len(self.decrypto_dat)):
					...
					if self.decrypto_dat[i][2]=='0x0':
						...
						if len(self.decrypto_dat[i]) == 6:
							self.decrypto_dat[i][4] = '<b><u>Server version:</u></b> %s'%( self.decrypto_dat[i][5])
						elif len(self.decrypto_dat[i]) == 7:
						...
						self.decrypto_dat[i].append('<i>Downloaded in:</i><br> '+self.decrypto_dat[i][3])
						...
						self.decrypto_dat[i].append('<i>(Updating app)</i><br><b><u>Update complete</u></b>')
						self.decrypto_dat[i].append('-')
						
					elif self.decrypto_dat[i][2] == '207':
						self.decrypto_dat[i].append('<i>(Updating app)</i><br><b><u>Zip removed</u></b>')
						self.decrypto_dat[i].append('-')
						...
						if len(self.decrypto_dat[i])==8:
							self.decrypto_dat[i] = '||'.join(self.decrypto_dat[i]).replace('Hashing', '||Hashing').split('||')
