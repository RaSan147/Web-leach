...
def get_comic_page_images(issue_dict):
    ...
    for index, page in enumerate(issue_dict['page-urls']):
        ...
        if args.number:
            outpath = "%s" % args.output + "/" + episodeNumber + "_%s_%s" % (issue_dict['author'], issue_dict['title'])
        else:
            outpath = "%s/%s_%s" % (args.output, issue_dict['author'], issue_dict['title'])
        episodeNumber = issue_dict['url'].split('episode_no=')[1]
