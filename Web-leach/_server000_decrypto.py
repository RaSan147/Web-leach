...
# import decrypto_5_4
import RcryptxAsuna2_1_c_py_lines as rcrypto
		...
		if self.path=='/':
			if self.is_get_req:
				...
				for i in dec_raw.replace('\r\n', '\n').split('\n')[::-1]:
					if i!='':self.decrypto_dat.append(rcrypto.PYdecrypt(i, decrypto_key)[40:].split('||')[:-1])
				...
				for i in range(len(self.decrypto_dat)):
					...
					if self.decrypto_dat[i][2].startswith('11000x'):
						if self.decrypto_dat[i][2][6]=='1':
							...
							self.decrypto_dat[i][4]= '<b><u>Project name:</u></b> %s <br><b><u>Project Status:</u> Was completed</b> '%self.decrypto_dat[i][3]
							...
							self.decrypto_dat[i][4]= '<b><u>Project name:</u></b> %s <br><b><u>Project Status:</u> Was Incompleted</b> '%self.decrypto_dat[i][3]
					...
					if self.decrypto_dat[i][2].startswith("10009x"):
						if self.decrypto_dat[i][2][6:]=='-1':
							...
							self.decrypto_dat[i][4]= 'Asking for inputs'
