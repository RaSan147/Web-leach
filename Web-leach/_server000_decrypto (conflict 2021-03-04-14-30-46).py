			...
			if self.is_get_req:
				...
				with open('data/userlog.leach') as f:
					...
					for i in f.read().replace('\r\n', '\n').split('\n')[::-1]:
						if i!='':self.decrypto_dat.append(decrypto_5_4.decrypt(i, decrypto_key)[40:].split('||')[:-1])
				...
				for i in range(len(self.decrypto_dat)):
					...
					if self.decrypto_dat[i][2]=='0x0':
						...
						self.decrypto_dat[i].append("No internet connection (for online mode) nor Update file found (for offline mode)")
